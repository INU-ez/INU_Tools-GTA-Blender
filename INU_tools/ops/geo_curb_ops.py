# -*- coding: utf-8 -*-
#
# INU_tools.ops.geo_curb_ops — процедурный бордюр, трава и тротуар на
# гео-нодах. Вкладка «GTA Geo», панель «Тротуар и бордюр»
# (см. ui/geo_panels.py).
#
# ЗАПУСК
#   Кнопка «Тротуар и бордюр» на выделенных меш-объектах. Собирает
#   нод-группу «INU_Curb» и вешает её модификатором.
#
# ВЕРСИЯ ГРУППЫ
#   Группа живёт в .blend пользователя, а не в аддоне. Поэтому в неё
#   штампуется SCRIPT_BUILD, и при расхождении с версией аддона группа
#   пересобирается сама — жать кнопку заново не нужно. Правишь сборку —
#   подними SCRIPT_BUILD, иначе у пользователя останется старая группа.
#
...
# Сколько строк — столько наборов полей в модификаторе. Набор входов
# гео-нод фиксируется при сборке группы и на лету не растёт, поэтому
# новая строка здесь требует подъёма SCRIPT_BUILD — тогда группа у
# пользователя пересоберётся сама.
ROUND_GROUPS = (
    ("curb_round_1", 0.3, 4),
    ("curb_round_2", 1.0, 6),
    ("curb_round_3", 3.0, 8),
...
# =============================================================================
# ОПЕРАТОРЫ И ВЕРСИОНИРОВАНИЕ ГРУППЫ
# =============================================================================

# Штамп версии сборки на самой нод-группе. Группа сохраняется в .blend
# пользователя и живёт там своей жизнью; аддон обновляется отдельно.
# По расхождению штампа с SCRIPT_BUILD видно, что группа старая.
BUILD_KEY = 'inu_curb_build'


def group_users(ng):
    """Объекты сцены, на которых висит модификатор с этой группой."""
    return [o for o in bpy.data.objects
            if any(m.type == 'NODES' and m.node_group is ng
                   for m in o.modifiers)]


def has_modifier(obj, ng):
    return any(m.type == 'NODES' and m.node_group is ng
               for m in obj.modifiers)


def _snapshot(obj, ng):
    """Значения модификатора ПО ИМЕНИ сокета, снятые до пересборки.

    В модификаторе значения лежат под идентификаторами сокетов, а
    идентификаторы при пересборке интерфейса меняются. Имя — единственное,
    что переживает пересборку, поэтому настройки пользователя сохраняем и
    возвращаем именно по нему. Иначе каждое обновление аддона сбрасывало бы
    ширину, высоту и материалы на всех тротуарах в файле."""
    snap = {}
    mod = next((m for m in obj.modifiers
                if m.type == 'NODES' and m.node_group is ng), None)
    if mod is None:
        return snap
    for item in ng.interface.items_tree:
        if getattr(item, 'item_type', 'SOCKET') != 'SOCKET':
        ...
        try:
            ...
            continue
        # Векторные поля приходят IDPropertyArray — он привязан к старому
        # модификатору, поэтому копируем в обычный список.
...
def _restore(mod, ng, snap):
    """Вернуть снятые значения по именам сокетов. Тип сокета мог смениться
    между версиями — тогда значение просто не встаёт, и поле остаётся со
    значением по умолчанию, что честнее подстановки мусора."""
    for item in ng.interface.items_tree:
        if getattr(item, 'item_type', 'SOCKET') != 'SOCKET':
            continue
        if item.in_out != 'INPUT' or item.socket_type in _NO_RESET:
            continue
        if item.name not in snap:
            continue
        try:
            mod[item.identifier] = snap[item.name]
        except (KeyError, TypeError, AttributeError, ValueError):
            pass


def ensure_group(force=False):
    """Группа нужной версии. Возвращает (группа, предупреждения, пересобрана).

    Готовую группу той же версии не трогаем: пересборка стирает значения
    модификаторов, а делать это на каждое нажатие кнопки незачем."""
    ng = bpy.data.node_groups.get(GROUP_NAME)
    if ng is not None and not force and ng.get(BUILD_KEY) == SCRIPT_BUILD:
        return ng, [], False
    ng, warn = build_group()
    ng[BUILD_KEY] = SCRIPT_BUILD
    return ng, warn, True


def apply_to(objects, force=False):
    """Собрать/обновить группу и повесить её на objects.

    Возвращает (группа, предупреждения, пересобрана, сколько объектов
    получили модификатор впервые)."""
    old = bpy.data.node_groups.get(GROUP_NAME)
    stale = old is None or force or old.get(BUILD_KEY) != SCRIPT_BUILD

    # Снимок ДО пересборки: после неё старый интерфейс уже стёрт и
    # сопоставить значения будет не с чем.
    snaps = {}
    if old is not None and stale:
        for ob in group_users(old):
            snaps[ob.name] = _snapshot(ob, old)

    ng, warn, rebuilt = ensure_group(force=force)

    if rebuilt:
        # Модификаторы продолжают ссылаться на ту же группу, но их значения
        # сидят на идентификаторах прежнего интерфейса — пересаживаем каждый
        # заново и возвращаем настройки по именам.
        for ob in group_users(ng):
            mod = attach(ob, ng)
            _restore(mod, ng, snaps.get(ob.name, {}))

    fresh = 0
    for ob in objects:
        if not has_modifier(ob, ng):
            attach(ob, ng)
            fresh += 1
    return ng, warn, rebuilt, fresh


@bpy.app.handlers.persistent
def curb_autoupgrade(_dummy):
    """Догнать версию группы при открытии файла — чтобы пользователю не
    приходилось помнить про кнопку после обновления аддона. Файлы без
    тротуара это не трогает вовсе."""
    ng = bpy.data.node_groups.get(GROUP_NAME)
    if ng is None or ng.get(BUILD_KEY) == SCRIPT_BUILD:
        return
    try:
        apply_to([], force=False)
        print("[INU Curb] группа обновлена до сборки %d" % SCRIPT_BUILD)
    except Exception as exc:                       # noqa: BLE001
        print("[INU Curb] автообновление не удалось: %s" % exc)


def _targets(context):
    """Выделенные меши, а если выделения нет — активный объект."""
...
class GTATOOLS_OT_geo_curb_apply(bpy.types.Operator):
    """Собрать процедурный тротуар с бордюром и травой на выделенных мешах.
Весь участок по умолчанию тротуар, трава — грани с материалом травы,
спуск к дороге — по рёбрам, помеченным острыми (Mark Sharp)"""
    bl_idname = "gtatools.geo_curb_apply"
    bl_label = "Тротуар и бордюр"
    bl_options = {'REGISTER', 'UNDO'}

    force: bpy.props.BoolProperty(
        name="Пересобрать группу",
        description="Собрать нод-группу заново, даже если версия совпадает. "
                    "Нужно, только если группу правили руками и она сломалась",
        default=False,
    )
    ...
    def execute(self, context):
        ...
        if not targets and not self.force:
            self.report({'WARNING'}, T("Выдели меш-объекты"))
            return {'CANCELLED'}

        ng, warn, rebuilt, fresh = apply_to(targets, force=self.force)

        # Заливать нечего — самая частая причина «ничего не появилось».
        empty = [o.name for o in targets if not o.data.polygons]
        if empty:
            self.report({'WARNING'},
                        "%s %s" % (T("Нет граней, заливать нечего:"),
                                   ", ".join(empty)))
        for problem in warn:
            print("[INU Curb] не выставилось: %s" % problem)

        if warn:
            self.report({'WARNING'},
                        "%s %d" % (T("Группа собрана с замечаниями, смотри "
                                     "системную консоль:"), len(warn)))
        elif rebuilt:
            self.report({'INFO'},
                        "%s %d" % (T("Группа пересобрана, сборка"),
                                   SCRIPT_BUILD))
        else:
            self.report({'INFO'},
                        "%s %d" % (T("Тротуар добавлен на объектов:"), fresh))
        return {'FINISHED'}


class GTATOOLS_OT_geo_curb_remove(bpy.types.Operator):
    """Снять модификатор тротуара с выделенных объектов.
Сама нод-группа остаётся в файле — тротуар можно вернуть кнопкой"""
    bl_idname = "gtatools.geo_curb_remove"
    bl_label = "Снять тротуар"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ng = bpy.data.node_groups.get(GROUP_NAME)
        if ng is None:
            self.report({'INFO'}, T("Тротуара в файле нет"))
            return {'CANCELLED'}
        count = 0
        for obj in _targets(context):
            for mod in list(obj.modifiers):
                if mod.type == 'NODES' and mod.node_group is ng:
                    obj.modifiers.remove(mod)
                    count += 1
        self.report({'INFO'}, "%s %d" % (T("Снято:"), count))
        return {'FINISHED'}
